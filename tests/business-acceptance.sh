...
# UTC until configured; foreign currency never converted; CSV without private
...
t "business time is UTC until the Founders configure it (needs founder action)" "UTC|False|NEEDS_FOUNDER_ACTION" "$(bj overview 'd["timezone"]["timezone"]')|$(bj overview 'str(d["timezone"]["configured"])')|$(bj overview 'd["timezone"]["status"]')"
...
t "an expected fee is kept apart from the actual one" "200|1440|1455" "$(cost '{"order_reference":"'"$M1REF"'","category":"PAYMENT_PROCESSING_FEE","basis":"EXPECTED","amount_minor":60}')|$(biz products; bj products '[p for p in d["products"]["products"] if p["sku"]=="moment"][0]["expected"]["contribution_minor"]')|$(bj products '[p for p in d["products"]["products"] if p["sku"]=="moment"][0]["actual"]["contribution_minor"]')"
...
t "  → known production cost only where recorded; pricing needs founder authorisation" "900|1 of 1 videos with a recorded cost (before payment fees and refunds)|4900,5900,6900" "$(bj videos 'd["video"]["production_cost"]["known_cost_minor"]')|$(bj videos 'd["video"]["known_contribution_based_on"]')|$(bj videos '",".join(str(p) for p in d["video"]["pricing_evidence"]["price_points_under_review_minor"])')"
...
biz products
t "keepsake: expected contribution awaits a payment fee (no fee model yet); actual = total − £48 supplier − £2 fee" "null|0|$((KTOTAL-4800-200))" "$(bj products '[p for p in d["products"]["products"] if p["sku"]=="keepsake-7-picture-disc"][0]["expected"]["contribution_minor"]')|$(bj products '[p for p in d["products"]["products"] if p["sku"]=="keepsake-7-picture-disc"][0]["expected"]["complete_orders"]')|$(bj products '[p for p in d["products"]["products"] if p["sku"]=="keepsake-7-picture-disc"][0]["actual"]["contribution_minor"]')"
with_config "\$c['business'] = ['payment_fee_model' => ['percent_basis_points' => 150, 'fixed_minor' => 20, 'currency' => 'GBP']];"
biz products
t "  → with the Founders' fee model the expected fee is calculated (1.5% + 20p) on £53 expected route costs (internal allowances counted once)" "$((KTOTAL-5300-(KTOTAL*150+5000)/10000-20))" "$(bj products '[p for p in d["products"]["products"] if p["sku"]=="keepsake-7-picture-disc"][0]["expected"]["contribution_minor"]')"
restore_config
biz suppliers
...
biz data
t "data quality shows the gaps: the USD order, analytics coverage, fee model, thresholds, timezone" "1|ANALYTICS_COVERAGE_INCOMPLETE|1|6|1" "$(bj data '[g["count"] for g in d["data_quality"]["gaps"] if g["key"]=="foreign_currency_orders"][0]')|$(bj data '[g["status"] for g in d["data_quality"]["gaps"] if g["key"]=="analytics_coverage"][0]')|$(bj data '[g["count"] for g in d["data_quality"]["gaps"] if g["key"]=="payment_fee_model"][0]')|$(bj data '[g["count"] for g in d["data_quality"]["gaps"] if g["key"]=="thresholds_not_configured"][0]')|$(bj data '[g["count"] for g in d["data_quality"]["gaps"] if g["key"]=="business_timezone"][0]')"
...
t "  → an invalid timezone falls back to truthful UTC" "UTC|False" "$(biz overview; bj overview 'd["timezone"]["timezone"]')|$(bj overview 'str(d["timezone"]["configured"])')"
restore_config
t "readiness shows the timezone, fee model and thresholds as founder actions" "NEEDS_FOUNDER_ACTION|NEEDS_FOUNDER_ACTION|NEEDS_FOUNDER_ACTION" "$(cc view=readiness >/dev/null; cj '[r["status"] for r in d["readiness"] if r["key"]=="business_timezone"][0]')|$(cj '[r["status"] for r in d["readiness"] if r["key"]=="payment_fee_model"][0]')|$(cj '[r["status"] for r in d["readiness"] if r["key"]=="commercial_thresholds"][0]')"
