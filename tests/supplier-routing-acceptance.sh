...
t "  → every catalogue physical SKU is mapped (33), including the Founders' 18 pop-up cards" "33|True|18" "$(cj 'len(d["registry"])' $SJ)|$(python3 -c 'import json;c=json.load(open("'"$CATALOGUE"'"))["skus"];s=json.load(open("'"$SJ"'"));print({k for k,v in c.items() if v["fulfilment"]=="PHYSICAL"}=={e["sku"] for e in s["registry"]})')|$(cj '[f["mapped"] for f in d["families"] if f["family"]=="CARD"][0]' $SJ)"
...
ROOTQ srb < db/migrations/2026-09-17-resilience.sql 2>/dev/null
...
t "without route data every product has no safe route; nothing is ready; research says NO_ROUTE" "False|33|0|33" "$(sj ov0 'str(d["route_data_present"])')|$(sj ov0 '[t["count"] for t in d["tiles"] if t["key"]=="products_without_safe_route"][0]')|$(sj ov0 '[t["count"] for t in d["tiles"] if t["key"]=="routes_ready"][0]')|$(sj ov0 'len([r for r in d["research"] if r["kind"]=="NO_ROUTE"])')"
t "  → the cards are known products (18 of 18 mapped): product data is not confused with route verification" "18|18|MAPPED|False|18" "$(sj ov0 'd["catalogue"]["cards"]["mapped"]')|$(sj ov0 'd["catalogue"]["cards"]["expected"]')|$(sj ov0 'd["catalogue"]["cards"]["status"]')|$(sj ov0 'str(any(r["kind"]=="FOUNDER_DATA_REQUIRED" for r in d["research"]))')|$(sj ov0 'len([r for r in d["research"] if r["kind"]=="NO_ROUTE" and (r["sku"] or "").startswith("pop-up-card-")])')"
...
t "route freshness is 30 days by founder decision: a 200-day-old verification is STALE (REVERIFY ROUTE)" "STALE|STALE|True|30" "$(sj k7a '[o for g in d["result"]["groups"].values() for o in g if o["route_id"]=="R-7-STALE-MARKET"][0]["verification"]["state"]')|$(sj k7a '[o for g in d["result"]["groups"].values() for o in g][0]["verification"]["freshness"]["status"]')|$(sj k7a 'str([o for g in d["result"]["groups"].values() for o in g][0]["verification"]["reverify"])')|$(sj k7a '[o for g in d["result"]["groups"].values() for o in g][0]["verification"]["freshness"]["freshness_days"]')"
with_config "\$c['fulfilment'] = ['route_freshness_days' => 90, 'new_sale_safety' => 'ADVISORY'];"
...
t "  → freshness (30 days), risks and an available fallback are shown" "FRESH|True|R-PIC-MARKET|true" "$(kd R-PIC-DIRECT '["verification"]["freshness"]["status"]')|$(kd R-PIC-DIRECT '["risks"]' | grep -q 'Test risk note: long production in December' && echo True || echo False)|$(kd R-PIC-DIRECT '["fallback"]["route_id"]')|$(kd R-PIC-DIRECT '["fallback"]["available"]')"
...
printf '%s' '{"currency":"GBP","pricing":{"PLAYER":"DESTINATION_CALCULATED","CARD":"DESTINATION_CALCULATED"},"rates":[{"id":"T_UK","label":"UK delivery (test table)","countries":["GB"],"first_item_minor":400,"additional_item_minor":100},{"id":"T_UK_PLAYER","label":"Player delivery (test table)","countries":["GB"],"classes":["PLAYER"],"first_item_minor":1200,"additional_item_minor":900},{"id":"T_UK_CARD","label":"Card delivery (test table)","countries":["GB"],"classes":["CARD"],"first_item_minor":300,"additional_item_minor":100}]}' > "$RATES"; settle
...
t "  → it needs every part: delivered cost, currency, availability, destination support and evidence" "CONFIRMED_DELIVERED_COST,AVAILABILITY_CONFIRMED,DESTINATION_SUPPORT_CONFIRMED,SUPPORTING_EVIDENCE" "$(docker exec mcb-api php -r 'require "/var/www/html/api/lib/bootstrap.php"; require "/var/www/html/api/lib/routing.php"; echo implode(",", array_column(route_authorisation_requirements(db(), (int) $argv[1])["unmet"], "requirement"));' $G 2>/dev/null)"
t "  → a route choice without the evidence does not unlock it; card details in the evidence are refused" "200|409|422|card_number_in_note" "$(decide $GREF antique-brass-gramophone R-GRAMO-HV)|$(founder_authorise $G LEWIS "$FOUNDER_CODE_LEWIS")|$(decide $GREF antique-brass-gramophone R-GRAMO-HV '{"confirmed_delivered_cost_minor":125000,"confirmed_delivered_currency":"GBP","delivered_cost_evidence":"paid with 4242 4242 4242 4242"}')|$(jget error)"
t "  → cost and currency alone are not enough: availability and destination support must also be confirmed" "200|409|AVAILABILITY_CONFIRMED,DESTINATION_SUPPORT_CONFIRMED|422|confirmation_evidence_required" "$(decide $GREF antique-brass-gramophone R-GRAMO-HV '{"confirmed_delivered_cost_minor":125000,"confirmed_delivered_currency":"GBP","delivered_cost_evidence":"Partner checkout total to the GB address (test)"}')|$(founder_authorise $G LEWIS "$FOUNDER_CODE_LEWIS")|$(docker exec mcb-api php -r 'require "/var/www/html/api/lib/bootstrap.php"; require "/var/www/html/api/lib/routing.php"; echo implode(",", array_column(route_authorisation_requirements(db(), (int) $argv[1])["unmet"], "requirement"));' $G 2>/dev/null)|$(decide $GREF antique-brass-gramophone R-GRAMO-HV '{"availability_confirmed":True,"destination_confirmed":True}')|$(jget error)"
t "  → with cost, currency, availability, destination support and evidence recorded, Lewis can authorise; nothing is purchased" "200|200|LEWIS|0" "$(decide $GREF antique-brass-gramophone R-GRAMO-HV '{"confirmed_delivered_cost_minor":125000,"confirmed_delivered_currency":"GBP","availability_confirmed":True,"destination_confirmed":True,"delivered_cost_evidence":"Partner checkout total to the GB address, in stock, screenshot filed (test)"}')|$(founder_authorise $G LEWIS "$FOUNDER_CODE_LEWIS")|$(q "SELECT supplier_purchase_authorised_by FROM order_production WHERE order_id=$G")|$(q "SELECT COUNT(*) FROM supplier_orders WHERE order_id=$G")"
...
to_ready '{"sku":"keepsake-7-picture-disc","players":[["pop-up-card-wedding",1]],"email":"sr-card@example.com"}'; CD=$OID
t "a real catalogue card rides with the order at its public price (£49.99)" "1|4999|CARD" "$(q "SELECT COUNT(*) FROM order_items WHERE order_id=$CD AND item_id='pop-up-card-wedding'")|$(q "SELECT unit_minor FROM order_items WHERE order_id=$CD AND item_id='pop-up-card-wedding'")|$(q "SELECT category FROM order_items WHERE order_id=$CD AND item_id='pop-up-card-wedding'")"
founder_authorise $CD BELLA "$FOUNDER_CODE_BELLA" >/dev/null
t "an alternative card needs the customer-impact assessment" "422|customer_impact_required" "$(act $CD RECORD_SUPPLIER_ORDER '"supplier_order_reference":"SO-SR-C1","skus":["pop-up-card-wedding"],"substitute_sku":"TEST alternative wedding card design (synthetic)","substitute_for_sku":"pop-up-card-wedding","substitution_note":"Original design out of stock; same occasion and style (test).","send_email":false')|$(jget error)"
t "  → with it, the original, alternative, reason, authority and impact are recorded" "200|pop-up-card-wedding|TEST alternative wedding card design (synthetic)|STAFF|Ops Tester|CUSTOMER_TOLD|1" "$(act $CD RECORD_SUPPLIER_ORDER '"supplier_order_reference":"SO-SR-C1","skus":["pop-up-card-wedding"],"substitute_sku":"TEST alternative wedding card design (synthetic)","substitute_for_sku":"pop-up-card-wedding","substitution_note":"Original design out of stock; same occasion and style (test).","customer_impact":"CUSTOMER_TOLD","customer_impact_note":"Customer emailed about the design change (test).","send_email":false')|$(q "SELECT original_sku FROM card_alternatives WHERE order_id=$CD")|$(q "SELECT alternative FROM card_alternatives WHERE order_id=$CD")|$(q "SELECT authority FROM card_alternatives WHERE order_id=$CD")|$(q "SELECT authorised_by FROM card_alternatives WHERE order_id=$CD")|$(q "SELECT customer_impact FROM card_alternatives WHERE order_id=$CD")|$(q "SELECT COUNT(*) FROM fulfilment_exceptions WHERE order_id=$CD AND type='AUTHORISED_CARD_ALTERNATIVE'")"
...
tc "SUPPLIER DATA NEEDS REVIEW lists shipping quote, destination, unavailable, template, capacity, fallback, no route" "$(sj ov 'str(all(k in {r["kind"] for r in d["research"]} for k in ["SHIPPING_QUOTE_MISSING","VERIFICATION_REQUIRED","PRODUCT_UNAVAILABLE","TEMPLATE_MISSING","CAPACITY_MISSING","FALLBACK_ABSENT","NO_ROUTE","COST_MISSING","DESTINATION_UNVERIFIED"]))' | grep -qx True && echo 1 || echo 0)"
...
t "Command Centre counts: routes ready (R-PIC-DIRECT, R-HEART, R-GATEFOLD, R-FRAME-DIRECT, R-GRAMO-V, R-PLAQUE; R-7 is stale) = 6" "6|$EXP_READY" "$(tile routes_ready)|$(tile routes_ready)"
t "  → needing verification: R-PIC-MARKET, R-10-NOSOURCE, R-10-FUTURE, R-10-UNSUP-NOEVIDENCE, R-FRAME-MARKET-A/B, R-FR14-MARKET/DIRECT, R-GRAMO-HV, stale R-7 = 10" "10" "$(tile routes_needing_verification)"
t "  → products with no safe route: 4 frames, journey-6, keepsake-10, keepsake-7 (stale), the portable player, the £1,000 gramophone and the 18 cards (no routes) = 27" "27|True" "$(tile products_without_safe_route)|$(sj ov 'str({i["sku"] for i in [t for t in d["tiles"] if t["key"]=="products_without_safe_route"][0]["items"]}=={"lyrics-frame-12x18","lyrics-frame-14x21","lyrics-frame-16x24","lyrics-frame-20x30","journey-6","keepsake-10-picture-disc","keepsake-7-picture-disc","portable-suitcase-record-player","antique-brass-gramophone"}|{e["sku"] for e in json.load(open("'"$SJ"'"))["registry"] if e["family"]=="CARD"})')"
