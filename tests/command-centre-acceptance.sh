...
t "  → system health: no problems found by these checks (never a blanket 'all good')" "NO_PROBLEMS_FOUND|No problems found by these checks" "$(ovj 'd["health"]["status"]')|$(ovj 'd["health"]["label"]')"
...
t "  → days are the MCB business day (Europe/London) and the week starts Monday" "Europe/London|Europe/London|1" "$(ovj 'd["period"]["timezone"]')|$(ovj 'd["revenue"]["timezone"]')|$(cc 'view=overview&period=week' >/dev/null; cj 'int(__import__("datetime").date.fromisoformat(d["period"]["local_date"]).weekday()==0)')"
