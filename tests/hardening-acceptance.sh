...
tc "77. earlier migrations are untouched; Sprint 4, Sprint 5, Single Creative Authority and Automation Foundation additive migrations follow" \
  "$([ "$(ls db/migrations/*.sql | wc -l | tr -d ' ')" = "18" ] && [ -f db/migrations/2026-09-16-business-intelligence.sql ] && git diff --quiet 720c556a -- db/migrations/2026-09-16-customer-care.sql && git diff --quiet 2634c1b1 -- db/migrations/2026-09-15-memory-music-video.sql && git diff --quiet 56c1ad93 -- db/migrations/2026-09-15-fulfilment-controller.sql && [ -f db/migrations/2026-09-15-fulfilment-controller.sql ] && git diff --quiet cf92949f -- db/migrations/2026-09-15-production-file-factory.sql && [ -f db/migrations/2026-09-15-single-creative-authority.sql ] && [ -f db/migrations/2026-09-15-creative-factory.sql ] && [ -f db/migrations/2026-09-15-production-file-factory.sql ] && git diff --quiet 25c428e1 -- db/migrations/2026-09-15-creative-factory.sql \
