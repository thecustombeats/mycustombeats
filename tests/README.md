...
Twenty suites, over 2,700 assertions, against a live PHP + MariaDB stack. Everything
...
# (the ops notice stays dormant). Set fulfilment.new_sale_safety 'ADVISORY' (suites that
# predate route data; supplier-routing and security set REQUIRED themselves). Founder authority and the outbox worker use
...
for s in api automation checkout command-centre creative delivery fulfilment full-package hardening legal lifecycle operations production release transaction video customer-care business supplier-routing security; do
...
| business | Business & Profit Intelligence: an empty business (zeros, awaiting data, NOT CONFIGURED thresholds, UTC, video 0/45 pending Mozart verification, contribution never called profit), Moment £15 and Moment + video £64, migration equivalence and preflight; live vs TEST vs USD (never converted); unknown costs never £0, entries refused where another record owns the cost, amount/currency/note/card rules, expected vs actual, voided history; video order incomplete until its cost is recorded; Moment and video attachment, product rows, enhancements (correlation, not cause); a physical order with route expected economics and actual supplier cost, the founders' fee model, route metrics staff-only and unranked; negative contribution flagged with review-only evidence, thresholds evaluated only when configured, alerts change no order, price, catalogue or suspension; partial and full refunds reduce net paid exactly, no refund request; replacement authorisation not spend until its cost is recorded; support burden and root causes without labour cost; customers, structured-only cruise and occasions, country codes without addresses, funnel coverage incomplete; data-quality gaps; complete-period comparisons, brief without celebration, timezone configured/invalid, readiness; CSV exports audited, formula-safe, without private content; no model, outbound call, refund or supplier purchase |
| supplier-routing | Supplier Intelligence & Commercial Routing (synthetic routes only): the 33-SKU registry (15 mapped, 18 cards FOUNDER DATA REQUIRED), vinyl prices and songs, exact sleeve and disc specifications with unknowns kept unknown, capacity UNVERIFIED, migration equivalence; no route data assumes nothing; verification calculated (no source, future date, unsupported without evidence, suspended, unavailable, geographic inconsistency held), staleness only when configured and a stale route never disables a paid order; the routing engine by customer destination (groups, RECOMMENDED FOR REVIEW with reasons, costs by component counted once, shipping and destination certainty, international evidence, fallback, determinism, unaffected by founder/server location), frames, the manual £1,000 gramophone and the plaque (never a country-only message); new-sale safety (unsupported destination → MCB confirms delivery, negative contribution advisory then REQUIRED, missing route, manufacturing data, high-risk marketplace) and suspension leaving paid orders untouched; route decisions with deviation reasons (never an authorisation), the delivered-cost gate, actual cost with tax/duty, split partner orders and parcels, a delivery problem; card alternatives with the full record and no broadening; research and manufacturing lists, Command Centre counts, scorecards (sample sizes, no combined score), the Business tab; founder decisions; privacy and no money movement |
| security | Security, Resilience & Automation Readiness: exposed surface and headers; PUBLIC/CUSTOMER/STAFF/FOUNDER separation (forged founders, wrong/missing codes, guessing across orders, staff-key throttle); cross-customer isolation (ids, tokens, expiry, revocation, cases, evidence, videos, signed links); uploads (disguised scripts, SVG, empty, corrupt, oversize, traversal names, duplicates, private headers); payment idempotency (double submit, refresh, duplicate/different/expired/forged/replayed events, underpayment, interrupted hand-over); failed and interrupted email, undelivered notifications and idempotent retries (money never retried); races (founder authorisation, partner order, route decision, dispatch, QC, support reply, last video slot); refund and supplier money boundaries; REQUIRED new-sale safety; pop-up cards; the £1,000 gramophone evidence gate; the complete migration chain from the original base, re-application, upgrade and preflight schema check; configuration audit, automation and founder action matrices; observability and privacy |
