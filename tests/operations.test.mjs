...
test("automation events are the named set, with nothing financial or supplier-facing", () => {
  assert.deepEqual([...M.ops.AUTOMATION_EVENTS].sort(), [
    ...
    // Creative Factory core
    "AUDIO.CAPACITY_CHECK_REQUIRED", "AUDIO.CAPACITY_EXCEPTION", "AUDIO.CAPACITY_PASSED",
    "CREATIVE.ALBUM_QC_REQUIRED", "CREATIVE.ALBUM_READY", "CREATIVE.CANDIDATE_READY", "CREATIVE.CREATIVE_QC_PASSED", "CREATIVE.CREATIVE_QC_REQUIRED",
    "CREATIVE.EXCEPTION", "CREATIVE.FACT_LEDGER_READY", "CREATIVE.FACT_QC_PASSED", "CREATIVE.GENERATION_REQUIRED", "CREATIVE.GENERATION_STARTED",
    "CREATIVE.JOB_READY", "CREATIVE.LYRICS_READY", "CREATIVE.LYRICS_REQUIRED", "CREATIVE.MASTER_READY", "CREATIVE.PLAN_READY", "CREATIVE.STORY_MAP_READY", "CREATIVE.TECHNICAL_QC_PASSED",
    // Production File Factory
    "ARTWORK.CREATIVE_JOB_READY", "ARTWORK.CREATIVE_MASTER_READY", "ARTWORK.VISUAL_QC_REQUIRED", "ARTWORK.VISUAL_QC_PASSED",
    "PRODUCTION.RENDER_REQUIRED", "PRODUCTION.MASTER_READY", "PRODUCTION.FILE_QC_PASSED",
    "MANUFACTURING.PACKAGE_REQUIRED", "MANUFACTURING.PACKAGE_READY", "MANUFACTURING.DATA_REQUIRED",
    // Fulfilment Controller (internal records; nothing is sent to a partner)
    "FULFILMENT.PARTNER_ORDER_RECORDED", "SHIPMENT.PARCEL_DISPATCHED", "SHIPMENT.IN_TRANSIT", "SHIPMENT.PARCEL_DELIVERED",
    "FULFILMENT.EXCEPTION_OPENED", "FULFILMENT.EXCEPTION_RESOLVED", "COMMERCIAL.ECONOMICS_CALCULATED",
    // MCB Memory Music Video (manual production; no platform is called)
    "VIDEO.ENTITLED", "VIDEO.CAPACITY_EXCEPTION", "VIDEO.INPUT_RECEIVED", "VIDEO.PRODUCTION_REQUIRED", "VIDEO.DURATION_PROVIDER_VERIFICATION_REQUIRED",
    "VIDEO.CANDIDATE_READY", "VIDEO.QUALITY_CHECK_REQUIRED", "VIDEO.REWORK_REQUIRED", "VIDEO.MASTER_READY", "VIDEO.REVEALED",
