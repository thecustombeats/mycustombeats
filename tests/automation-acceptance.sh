...
ROOTQ afb < db/migrations/2026-09-15-fulfilment-controller.sql 2>/dev/null
ROOTQ afb < db/migrations/2026-09-15-memory-music-video.sql 2>/dev/null
ROOTQ afb < db/migrations/2026-09-16-customer-care.sql 2>/dev/null
ROOTQ afb < db/migrations/2026-09-16-business-intelligence.sql 2>/dev/null
ROOTQ afb < db/migrations/2026-09-16-supplier-routing.sql 2>/dev/null
ROOTQ afb < db/migrations/2026-09-17-resilience.sql 2>/dev/null
...
t "every founder notification type is defined" "ARTWORK_EXCEPTION,AUDIO_CAPACITY_EXCEPTION,COMMERCIAL_SAFETY_EXCEPTION,CREATIVE_EXCEPTION,CUSTOMER_SUPPORT_EXCEPTION,DELIVERY_EXCEPTION,FULFILMENT_APPROVAL_REQUIRED,FULFILMENT_EXCEPTION,MANUFACTURING_DATA_REQUIRED,NEW_ORDER_READY_FOR_PROCESSING,PRODUCT_SALES_SUSPENDED,QC_EXCEPTION,VIDEO_CAPACITY_ALERT,VIDEO_EXCEPTION" "$(python3 -c 'import json;print(",".join(sorted(n["type"] for n in json.load(open("public/api/data/operations.json"))["founder_notifications"])))')"
