...
CREATE TABLE IF NOT EXISTS orders (
  ...
  -- The goods, and delivery, that make up total_minor on personalised orders.
  -- total_minor is the PAYABLE total: what checkout charges and the webhook
  -- matches.
  subtotal_minor         INT UNSIGNED  NULL,
  delivery_minor         INT UNSIGNED  NULL,
  -- How delivery was quoted (public/api/lib/delivery.php). UNAVAILABLE means
  -- no authorised rate exists for the destination, and checkout is refused.
  -- TEST_ONLY_FIXTURE can only ever be written by a server using a Stripe
  -- test key with test fixtures explicitly enabled.
  delivery_status        ENUM('NOT_REQUIRED','QUOTED','UNAVAILABLE') NULL,
  delivery_rate_source   ENUM('TEST_ONLY_FIXTURE','RATE_TABLE') NULL,
  delivery_rate_id       VARCHAR(64)   NULL,
  delivery_label         VARCHAR(120)  NULL,
  -- Per-memory personalisation (order_units / order_memories). Checkout for
  -- a song experience requires COMPLETE: every memory saved and every photo
  -- the customer said they would add received.
  personalisation_status ENUM('NOT_PROVIDED','AWAITING_UPLOADS','COMPLETE')
  ...
  -- PENDING: saved, awaiting payment (READY_FOR_CHECKOUT once personalisation
  -- is COMPLETE; PAYMENT_PENDING while a checkout session is open).
  -- PAYMENT_REVIEW: money arrived for this order but not the amount or
  -- currency expected — filed in unreconciled_payments for a person; never
  -- payable again online. CANCELLED: operator use.
  status                 ENUM('PENDING','PAID','PAYMENT_REVIEW','ABANDONED','REFUNDED','CANCELLED')
  ...
  -- The Stripe mode the payment was made in: 0 test, 1 live.
...
CREATE TABLE IF NOT EXISTS delivery_addresses (
  ...
  -- ISO 3166-1 alpha-2, the code delivery is quoted on.
...
CREATE TABLE IF NOT EXISTS unreconciled_payments (
  ...
  -- MODE_MISMATCH: a payment made in the other Stripe mode (a live payment
  --                reaching a test-configured server, or the reverse).
  -- ORDER_NOT_PAYABLE: an exact payment for an order that is no longer
  --                awaiting payment (under review, cancelled, refunded).
...
CREATE TABLE IF NOT EXISTS checkout_sessions (
  ...
  currency             CHAR(3)       NOT NULL DEFAULT 'GBP',
  -- The Stripe mode the session was created in: 0 test, 1 live.
...
-- ---------------------------------------------------------------------
-- order_units, order_memories, order_uploads, order_events
-- ---------------------------------------------------------------------
-- Personalised order persistence (release-candidate Sprint 4). One unit per
-- song product, plaque and lyrics frame; one memory per song or chapter; the
-- customer's photos stored privately and attached to what they are for; and
-- an append-only audit trail. See
-- db/migrations/2026-09-14-sprint4-order-persistence.sql.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Sprint 5 — operations after payment
-- ---------------------------------------------------------------------
-- Applied as the same statements as
-- db/migrations/2026-09-14-sprint5-operations.sql, so a fresh install and a
-- migrated database end in exactly the same shape. See that file and
-- src/data/operations.ts for what each column and table is for.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Applied as the same statements as
-- db/migrations/2026-09-15-automation-foundation.sql, so a fresh install
-- and a migrated database end in exactly the same shape.
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- Creative Factory core (15 September 2026)
-- ---------------------------------------------------------------------
-- Applied as the same statements as
-- db/migrations/2026-09-15-creative-factory.sql.
-- ---------------------------------------------------------------------
...
-- =====================================================================
-- Memory Music Video (15 September 2026) — see db/migrations/2026-09-15-memory-music-video.sql
-- =====================================================================
...
-- =====================================================================
-- Customer Care & Recovery Controller (16 September 2026) — see db/migrations/2026-09-16-customer-care.sql
-- =====================================================================
...
-- =====================================================================
-- Business & Profit Intelligence (16 September 2026) — see db/migrations/2026-09-16-business-intelligence.sql
-- =====================================================================
