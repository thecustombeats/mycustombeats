/**
 * Countries a delivery address may be in: ISO 3166-1 alpha-2 codes (plus XK,
 * Kosovo, which Stripe accepts), with English names.
 *
 * The CODE is what the server stores and what delivery is quoted on; the name
 * is for people. A free-text country cannot be matched to a delivery rate, so
 * the order form offers this list rather than a text box.
 *
 * NOT A LIST OF PLACES MCB DELIVERS TO AT A KNOWN PRICE. Whether a rate exists
 * for a destination is the server's answer (public/api/lib/delivery.php), and
 * no production rate table has been authorised yet.
 *
 * Compiled into public/api/data/personalisation.json for the server.
 */
