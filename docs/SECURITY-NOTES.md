...
| `crm/business` | GET/POST | CRM key and staff name; no-store, noindex; management figures only for founders and staff (no public financial API); CSV exports audited (`business_audit_log`), formula-safe, aggregates and order references only; cost entries refuse card-like text and categories owned by other records; no outbound call, model or payment action |
| `crm/*` (all 24 endpoints) | GET/POST | CRM key (Bearer, constant-time compare); no browser cookie |
