...
| `GET/POST /api/crm/support` | Customer Care (staff name required; case views audited): `?view=list&filter=new\|needs_mcb\|waiting_customer\|urgent\|resolved\|all`, `?case_id=`, `?view=metrics`, `?view=health`, `?view=templates`. POST {action, case_id, staff}: `RESPOND` {body, template_key?, next_status, send_email?, recovery_outcome?, root_cause?}, `ADD_INTERNAL_NOTE` {body}, `SET_STATUS` {status}, `SET_PRIORITY` {priority}, `ASSIGN` {assigned_staff}, `SET_SENTIMENT` {sentiment}, `SET_CUSTOMER_SUMMARY` {customer_summary}, `CLASSIFY` {classification, note}, `FLAG_PRIVACY_REVIEW` / `COMPLETE_PRIVACY_REVIEW` {note}, `RESOLVE` {recovery_outcome, root_cause?, resolution_note?}, `CLOSE`, `REISSUE_ORDER_LINK`, `PROPOSE_REMEDY` {type, costs_mcb?, capacity_basis?, video_job_id?, note?}, `DECIDE_REMEDY` {remedy_id, decision AUTHORISE\|DECLINE, note, founder, founder_code, confirm}, `START_REMEDY` / `COMPLETE_REMEDY` {remedy_id, note} / `CANCEL_REMEDY`, `REQUEST_REFUND_REVIEW` {refund_type FULL\|PARTIAL, amount_minor?, reason}, `SUBMIT_REFUND_FOR_DECISION` {refund_id}, `DECIDE_REFUND` {refund_id, decision, note, founder, founder_code, confirm}, `RECORD_REFUND` {refund_id, refunded_on, external_reference?}; `OPEN_CASE_FROM_EXCEPTION` {exception_id, note?}. No refund, payout or purchase API is called |
| `GET/POST /api/crm/business` | Business & Profit Intelligence (staff name required; no-store, noindex): `?section=overview\|products\|videos\|customers\|suppliers\|support\|data`; `?export=products\|supplier_routes\|refunds\|alerts\|root_causes\|geography\|data_quality` (CSV, audited, aggregates and references only); `?cost_target=MCB-…`. POST `RECORD_DIRECT_COST` {order_reference, category PAYMENT_PROCESSING_FEE\|VIDEO_PRODUCTION_COST (expected)\|REPLACEMENT_COST\|OTHER_DIRECT_COST, basis, amount_minor, currency?, remedy_id?, note?} and `VOID_DIRECT_COST` {entry_id, reason}. Nothing is spent, refunded, purchased or repriced |
...
| Endpoint | Purpose |
|---|---|
| `POST /api/order-support` | "Need help with your order?": {token, kind, item?, issue?, otherCustomerDetails?, priorityReplacement?, description} opens a support case (kinds filtered to what the order contains) |
| `POST /api/order-support-case` | The customer's side of a case: {token, case_id, action: message, message} or {token, case_id, action: satisfaction, answer YES\|NO} (another order's case → 404) |
